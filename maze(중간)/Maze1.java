...
//		System.out.println( "stackPop: [ " + cod[ 0 ] + " ][ " + cod[ 1 ] + " ]" );
	...
	public static void checkFourDirections( int maze[ ][ ], int coord[ ] ) {
		...
		stackPush( coord );
		// find an exit: success														// i tried to put print method in the finding process, tried to pop out the stack, but it did not work
																						// had trouble changing a reference defining a static value...
		if( row == exitRow && col == exitCol ) {												/* XXXX */
				System.out.println( "\n\n----------" );													 
