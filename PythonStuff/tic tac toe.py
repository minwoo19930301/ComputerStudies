...
def play_game():
    ...
    while not all_squares_filled():
        ...
        choice =input(get_name(current_player) + ", which square? (e.g. 2B, 2b, B2 or b2) ")
        ...
        if choice[0] not in ["1", "2", "3"] and (choice[0]).upper() not in [A, B, C]:
            print ("The first character must be a row (1, 2 or 3) or column (A, B or C).")
            print()
            continue

        if choice[1] not in ["1", "2", "3"] and (choice[1]).upper() not in [A, B, C]:
